...
-- does not validate output quantities. 'MaryValue' names the source
-- representation, not the era of the transaction.
-- Unlike V3/V4 @txInfoMint@, output values retain Ada in every version.
fromLedgerMaryValue :: MaryValue -> PV1.Value
fromLedgerMaryValue (MaryValue coin assets) = transCoinToValue coin <> fromLedgerMultiAsset assets
