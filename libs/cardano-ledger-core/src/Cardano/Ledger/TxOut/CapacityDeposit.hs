...
module Cardano.Ledger.TxOut.CapacityDeposit (
  CapacityDeposit (..),
) where
...
newtype CapacityDeposit = CapacityDeposit {unCapacityDeposit :: Coin}
  ...
  deriving newtype (NFData, NoThunks, EncCBOR, DecCBOR, ToJSON)

instance MemPack CapacityDeposit where
  packedByteCount (CapacityDeposit (Coin amount)) = packedByteCount amount
  packM (CapacityDeposit (Coin amount)) = packM amount
  unpackM = CapacityDeposit . Coin <$> unpackM
