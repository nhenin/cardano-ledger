...
* Add `EraTxOut.outputPotValueEitherTxOutF` and derived read-only `outputPotValueTxOutF`, `outputPotCoinsTxOutF`, and `compactOutputPotCoinsTxOutF` for the `OutputPotValue` / `OutputPotCoins` accounting projections; the default preserves existing eras
